        ...
        <div className="md:hidden border-t border-border/60 bg-ink/95 backdrop-blur-xl">
          <nav className="mx-auto max-w-7xl px-6 py-4 flex flex-col gap-1 text-sm">
            ...
            <div className="flex items-center gap-4 mt-4 px-3 py-3 border-t border-border/60">
              <a href={settings?.find((s: any) => s.key === "site_insta")?.value || "https://www.instagram.com/marketing2ponto0"} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-gold transition"><Instagram className="h-5 w-5" /></a>
              <a href={settings?.find((s: any) => s.key === "site_fb")?.value || "https://www.facebook.com/marketing2ponto0"} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-gold transition">
              ...
              <a href={settings?.find((s: any) => s.key === "site_linkedin")?.value || "https://www.linkedin.com/company/marketing2ponto0"} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-gold transition">
              ...
              <a href={settings?.find((s: any) => s.key === "site_tiktok")?.value || "https://www.tiktok.com/@marketing2ponto0"} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-gold transition">
      ...
      <div className="mx-auto max-w-7xl px-6 flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-muted-foreground">
        ...
        <div className="flex gap-6">
          ...
          <div className="flex items-center gap-4 ml-4 pl-4 border-l border-border/60">
            <a href={settings?.find((s: any) => s.key === "site_insta")?.value || "https://www.instagram.com/marketing2ponto0"} target="_blank" rel="noreferrer" className="hover:text-gold transition"><Instagram className="h-4 w-4" /></a>
            <a href={settings?.find((s: any) => s.key === "site_fb")?.value || "https://www.facebook.com/marketing2ponto0"} target="_blank" rel="noreferrer" className="hover:text-gold transition">
              <svg viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
            </a>
            <a href={settings?.find((s: any) => s.key === "site_linkedin")?.value || "https://www.linkedin.com/company/marketing2ponto0"} target="_blank" rel="noreferrer" className="hover:text-gold transition">
              <svg viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
            </a>
            <a href={settings?.find((s: any) => s.key === "site_tiktok")?.value || "https://www.tiktok.com/@marketing2ponto0"} target="_blank" rel="noreferrer" className="hover:text-gold transition">
